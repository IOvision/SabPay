...
public class OrderAdapter extends RecyclerView.Adapter<OrderAdapter.OrderViewHolder> {
    ...
    public void onBindViewHolder(@NonNull OrderAdapter.OrderViewHolder holder, int position) {
        ...
        if(holder.status.getText().toString().equalsIgnoreCase(Order.STATUS.ORDER_PLACED)){
            holder.status.setTextColor(Color.RED);
        } else if(holder.status.getText().toString().equalsIgnoreCase(Order.STATUS.ORDER_DELIVERED)) {
            holder.status.setTextColor(Color.GREEN);
        } else if(holder.status.getText().toString().equalsIgnoreCase(Order.STATUS.ORDER_RECEIVED)) {
